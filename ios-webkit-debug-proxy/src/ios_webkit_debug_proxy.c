...
ws_status iwdp_stop_devtools(iwdp_ipage_t ipage) {
  ...
  iwdp_iport_t iport = iws->iport;
  iwdp_iws_t iws2 = (iwdp_iws_t)ht_get_value(iport->ws_id_to_iws, sender_id);
  if (iws != iws2) {
    return WS_ERROR; // internal error?
  }
  iwdp_iwi_t iwi = iport->iwi;
...
wi_status iwdp_remove_app_id(wi_t wi, const char *app_id) {
  iwdp_iwi_t iwi = (iwdp_iwi_t)wi->state;
  ht_t app_id_ht = iwi->app_id_to_true;
  char *old_app_id = (char *)ht_get_key(app_id_ht, app_id);
  if (!old_app_id) {
    return WI_SUCCESS;
  }
...
wi_status iwdp_on_applicationSentData(wi_t wi,
  ...
  iwdp_iws_t iws = (iwdp_iws_t)ht_get_value(iport->ws_id_to_iws, dest_id);
  if (!iws) {
...
wi_status iwdp_on_applicationUpdated(wi_t wi,
    const char *app_id, const char *dest_id) {
  wi_status res = iwdp_remove_app_id(wi, app_id);
  if (res) {
    return res;
  }
  return iwdp_add_app_id(wi, dest_id);
}
...
iwdp_iwi_t iwdp_iwi_new(bool is_sim, bool *is_debug) {
  ...
  wi->on_applicationUpdated = iwdp_on_applicationUpdated;
  wi->state = iwi;
